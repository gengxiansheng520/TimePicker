...
import com.example.timepicker.adapters.NumericWheelAdapter;
import com.example.timepicker.config.PickerConfig;
import com.example.timepicker.data.source.TimeRepository;
import com.example.timepicker.utils.PickerContants;
import com.example.timepicker.utils.Utils;
...
public class TimeWheel {
    ...
    void updateMinutes() {
        ...
        int tempMinMinute = mRepository.getMinMinute(curYear, curMonth, curDay, curHour);
        int minMinute = tempMinMinute;
        if (tempMinMinute%5 != 0) {
            minMinute = (tempMinMinute / 5 + 1) * 5;
        }
        int maxMinute = mRepository.getMaxMinute(curYear, curMonth, curDay, curHour);
        mMinuteAdapter = new NumericWheelAdapter(mContext, minMinute, maxMinute, PickerContants.FORMAT, mPickerConfig.mMinute, 5);
